...
export const ThirdTermResult = ({ targetYear }) => {
    ...
    const fetchResults = async() => {
        try {
            // Filter specifically for "Third Term"
            const { data, error } = await supabase
    ...
    const handleUpload = async (studentId) => {
        ...
        // CHECK IF THIRD TERM RESULT ALREADY EXISTS FOR THIS LIVE ADMIN SESSION
        ...
        if (existing) return Swal.fire("Denied", "3rd Term result already uploaded for this session!", "error");
        ...
        try {
            ...
            if (error) throw error;
            Swal.fire("Success", `3rd Term result uploaded!`, "success");
            setAdd(""); 
            await fetchResults();
        } catch (error) {
            Swal.fire("Error", error.message, "error");
    ...
    const handleEdit = async (result) => {
        ...
        if (formValues) {
            ...
            try {
                ...
                if (error) throw error;
                Swal.fire("Updated", "Scores updated!", "success");
                await fetchResults();
            } catch (error) {
                Swal.fire("Error", error.message, "error");
            } finally {
            ...
            {loading && (
                <div className='fixed inset-0 bg-white/60 z-9999 flex items-center justify-center backdrop-blur-[1px]'>
                    <div className='flex flex-col items-center gap-2'>
                        ...
                        <span className='font-bold text-blue-600 animate-pulse'>Finalizing 3rd Term...</span>
                    </div>
                </div>
            )}

            <div>
                <h2 className='text-xl font-bold'>Third Term (Promotional)</h2>
            </div><hr className='w-60' />
                ...
                <div className='flex gap-3 overflow-x-auto pb-2'>
                    {classArray.map((items) => (
                        <div key={items.id} className='grid'>
                            ...
                            }} className={`border px-3 h-9 mt-1 rounded-xl transition-all whitespace-nowrap ${selectedClass === items.class_id && selectedSubjectId === items.subject_id ? 'bg-blue-600 text-white shadow-md' : 'border-slate-300 text-slate-400'
                    ...
                    <div key={item.id} className='shadow-sm shadow-slate-500/60 p-2 rounded-xl bg-white'>
                        <div className='flex justify-between gap-3'>
                            <div className='flex gap-4 w-md justify-between'>
                                ...
                                <span className='my-auto text-blue-600 font-semibold'>{item.full_name}</span>
                       ...
                       {add === item.id && (
                            <form className='mt-2 border-t pt-3'>
                                <div className='flex flex-col gap-4'>
                                    <div className='flex flex-wrap gap-4 items-center'>
                                        <div className='p-2 rounded-lg bg-blue-50/50 border border-blue-100'>
                                            <p className='text-[10px] font-bold text-blue-500 uppercase'>Subject</p>
                                        ...
                                        <div className='flex gap-4'>
                                            <div>
                                                ...
                                                <input
                                                    type="number"
                                                    className='border h-8 text-center rounded w-12 outline-none focus:border-blue-500'
                                ...
                                <div className='flex justify-end mt-3'>
                                    <button
                                        ...
                                        className='bg-blue-600 px-6 h-10 text-white rounded-xl font-bold active:scale-95 transition-all shadow-md'
                                    >
                                        Submit Final Term
                                    </button>
                                </div>
                        ...
                        <div className='md:me-4 py-2 px-1 shadow-sm mt-2 rounded-xl'>
                            <div className='flex gap-2 text-slate-500 justify-around'>
                                <button className='mb-1 flex font-bold py-1 gap-1 text-xs hover:text-blue-600'><FaGraduationCap size={18} className='my-auto' /> Profile</button>
                                <button className='mb-1 flex font-bold py-1 gap-1 text-xs hover:text-blue-600'  onClick={()=> setViewresults(viewResults === item.id ? " " : item.id)}
                                    ><FaFile size={16} className='my-auto'/> 3rd Term History</button>
                            </div>
                        </div>

                         {viewResults === item.id && (
                            <div className='p-2'>
                                <h2 className='text-slate-400 font-bold my-2 text-xs uppercase'>Third Term Result History</h2>
                            <div className='flex flex-col gap-2'>
                              {results.filter(result=> result.student_id === item.id && result.subject_id === selectedSubjectId).map((res)=> (
                               <div key={res.id} className='flex gap-2.5 justify-around border-b border-b-slate-300 p-2 items-center'>
                                 ...
                                 <div className='flex gap-4'>
                                    <button onClick={() => handleEdit(res)} className='text-blue-400 hover:text-blue-600 flex gap-1'><FaEdit className='my-1'/> Edit</button>
                                    <button onClick={() => handleDelete(res.id)} className='text-red-400 hover:text-red-600 flex gap-1'><FaTrash className='my-1'/> Delete</button>
                                 </div>
                               </div>
                            ))}
                          </div>
