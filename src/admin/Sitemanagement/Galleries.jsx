...
import { FaCloudUploadAlt, FaSpinner, FaTag, FaTrash } from 'react-icons/fa'
...
export const Galleries = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file.size > maxSize) {
      Swal.fire('Too Big!', 'Please select an image under 2MB.', 'error')
      e.target.value = "" 
      setPreviewUrl(null)
      setImageFile(null)
      return 
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className='flex justify-between items-center mb-8 bg-white p-2 rounded-3xl shadow-sm border border-slate-100'>
        <button onClick={() => setViewMode('upload')} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${viewMode === 'upload' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>Add New Photo</button>
        <button onClick={() => setViewMode('manage')} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${viewMode === 'manage' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>Manage Gallery</button>
      </div>
        ...
        <div className="p-8 bg-white rounded-2xl shadow-xl border border-slate-100 max-w-2xl mx-auto">
           ...
           <form onSubmit={handleUpload} className="space-y-6">
             <div className='space-y-2'>
                <label className='text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest'>Photo Title</label>
            ...
            {fetchLoading ? (
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredItems.map(item => (
                        <div key={item.id} className='bg-white rounded-4xl p-3 shadow-sm border border-slate-50'>
                            <div className='h-48 rounded-3xl overflow-hidden relative'>
                                ...
                                <div className='absolute top-3 left-3 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[8px] font-black uppercase text-blue-950'>{item.role}</div>
                            </div>
                            <div className='p-4 flex justify-between items-center'>
                                ...
                                <button onClick={() => handleDelete(item.id, item.image_url)} className='w-10 h-10 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-600 hover:text-white transition-all'><FaTrash /></button>
                            </div>
                        </div>
                    ))}
                </div>
            )}
            {filteredItems.length === 0 && !fetchLoading && <p className='text-center text-slate-400 font-bold uppercase text-xs py-20'>No photos found</p>}
        </div>
      )}
    </div>
  )
