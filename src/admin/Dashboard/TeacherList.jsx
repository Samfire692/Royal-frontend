...
export const TeacherList = () => {
        ...
        const [teacherInfo, setTeacherinfo] = useState([]); // Changed name for clarity
         ...
         const fetchInfo = async () => {
            ...
            const blockAdmin = async (id, status) => {
                ...
                const blockResult = await Swal.fire({
                    icon: "warning",
                    title: `${actionText} Teacher?`, // Changed to Student
            ...
            const filteredStudents = teacherInfo.filter((teacher) =>
                teacher.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                teacher.special_id?.toLowerCase().includes(searchTerm.toLowerCase())
            );
                ...
                <div className='displayAdmin p-3'>
                    ...
                    <div className='grid lg:grid-cols-3 mt-5 gap-4'>
                        {filteredStudents.map((teacher) => (
                            <div key={teacher.id} className={`shadow-sm w-full shadow-slate-500 rounded-4xl mx-auto p-3 transition-all ${teacher.status === 'blocked' ? 'opacity-70 bg-red-50' : ''}`}>
                                <div>
                                    <div className='flex justify-between'>
                                        <img src={teacher.profile_pic_url || profilepic} alt="" className='w-23 h-23 object-cover rounded-full' />
                                        <button onClick={() => blockAdmin(teacher.id, teacher.status)} className={`px-3 py-1 rounded-md text-sm font-medium transition-colors h-fit my-auto ${teacher.status === 'blocked' ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                                        >
                                            {teacher.status === 'blocked' ? 'Unblock' : 'Block'}
                                        </button>
                                    </div>
                                    <p className='mt-4 text-xl'>{teacher.full_name}</p>
                                    <p className='text-slate-500/80 mt-2'>{teacher.special_id}</p>
                                </div>
                                ...
                                <div className=''>
                                    ...
                                    <div className='flex gap-2'>
                                        ...
                                        <span>{teacher.phone_number || "no phone added yet"}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
